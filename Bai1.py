...
# --- Chỉ in ra các Pokémon có type là 'Poison' ---
print("\nCác Pokémon có hệ 'Poison':\n")
for pokemon in data:
    if "poison" in pokemon.get("type", []):
        types = ", ".join(pokemon["type"])
        print(f"{pokemon['number']}: {pokemon['name']} - Types: {types} - Height: {pokemon['height']} - Weight: {pokemon['weight']}")
